...
import { SiReact, SiGithub, SiRedux, SiHtml5, SiCss3, SiNextdotjs, SiExpress, SiNestjs, SiGithubactions, SiGitforwindows, SiJavascript } from "react-icons/si";
...
export const LogoOrigami = ({logos}: {logos:typeof LOGOS[0][]}) => {
    const LogoCom = useMemo(() => {
        const com = shuffleArray(logos).map((logo, idx) => <LogoItem key={idx} className={cn("text-neutral-800 bg-orange-500",logo.bg)}>
            {logo.logo}
        </LogoItem>);
        return com;
    }, [])
